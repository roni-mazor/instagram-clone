...
export default function Post({ id, username, userImg, img, caption, session }: Props) {
    ...
    if (postComments.length) console.log(postComments[0].data())
    ...
    return (
        <li className="bg-white my-7 border rounded-md">
            ...
            {/* Button set */}
            {session && <div className="flex justify-between px-3 pt-3">
                <div className="flex space-x-4">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 heart post-btn">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
                    </svg>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 chat-icon post-btn">
            ...
            {session && <form className="flex items-center p-3 space-x-4">
                ...
                <input value={comment} onChange={(e) => setComment(e.target.value)} type="text" className="flex-1 border-none outline-none " placeholder="Enter your Comment" />
